...
if __name__ == "__main__":
    """
    Script for executing the experiments with hybrid models in the FAR-Trans paper.
    For the experiments with the rest of the models, please, see run_hybrid_experiments_script.py.
    
    This program takes the FAR-Trans dataset and runs a series of hybrid recommender systems over
    the data that use the output of other models as features.
    ...
    algorithms = [("random", "random"),
                  ...
                  ("pop", "pop"),
                  ("arm", "arm"),
                  ("ubknn", "cas", "cosine", "16", "False"),
                  ("mf", "mf", "128", "bpr", "mf/backup"),
                  ("lightgcn", "lightgcn", "64", "0.01", "lightgcn/backup")]

    for date in dates:
        for algorithm in algorithms:
            print("Starting algorithm " + algorithm[1] + " for " + date[4] + " month(s)")

            directory = os.path.join(date[5], algorithm[1])
            if not os.path.exists(directory):
                os.makedirs(directory)

            exec_code = "python ./recommendation.py " + interactions_file + " " + time_series + " range " + \
                        date[0] + " " + date[1] + " " + date[2] + " " + date[3] + " " + os.path.join(date[5], algorithm[
                1]) + " " + date[4] + " None "
            ...
            if os.system(exec_code) != 0:
                sys.exit("Error when executing algorithm " + str(algorithm) + " for date " + str(date))

            print("Ending algorithm " + algorithm[1] + " for " + date[4] + " month(s)")
