...
class NoFilter(AssetFilter):
    ...
    def filter(self, time_series: DataFrame, train: DataFrame, valid: DataFrame, test: DataFrame, split_date) -> Set:
        assets = set(train[DEFAULT_ITEM_COL].unique().flatten())
        assets = assets | set(test[DEFAULT_ITEM_COL].unique().flatten())
        assets = assets | set(time_series[DEFAULT_ITEM_COL].unique().flatten())

        if valid is not None:
            assets = assets | set(valid[DEFAULT_ITEM_COL].unique().flatten())
        return assets
