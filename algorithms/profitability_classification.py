...
class ProfitabilityClassification(Algorithm):
    """
    Algorithm that classifies assets into profitable and non-profitable assets.
    Ranking is then produced by the probability that the asset is profitable, according to the algorithm.
    """
    def __init__(self, model, data, months, indicators):
        """
        Configures the profitability classification model.
    ...
    def train(self, train_date):
        ...
        for asset in kpi_indicators[DEFAULT_ITEM_COL].unique():
            ...
            asset_df["target"] = asset_df["target"].apply(lambda x: 1 if x > 0.0 else -1)
    ...
    def recommend(self, rec_time, repeated, only_test_customers):
        ...
        kpi_indicators["score"] = self.model.predict_proba(kpi_indicators.drop(columns=[DEFAULT_ITEM_COL, DEFAULT_TIMESTAMP_COL]))[1]
